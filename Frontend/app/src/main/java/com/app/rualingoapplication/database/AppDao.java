...
public interface AppDao {
    ...
    List<Vocabulary> getVocabularyByLesson(long lessonId);
    ...
    @Insert
    void insertPendingLog(PendingLog log);

    @Query("SELECT * FROM pending_logs")
    List<PendingLog> getAllPendingLogs();

    @Query("DELETE FROM pending_logs WHERE id = :logId")
    void deletePendingLog(int logId);
}
